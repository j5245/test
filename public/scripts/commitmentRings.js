...
window.onload = async () => {
  ...
  logEvent(analytics, 'view_commitmentRing_page', {
    page_name: 'Commitment Rings Collection'
  });

  if (typeof window.fetchCommitmentRingItems !== "function") {
  ...
  const fetchItems = async () => {
    ...
    try {
      const jewelryItems = await window.fetchCommitmentRingItems(currentPage, itemsPerPage);
