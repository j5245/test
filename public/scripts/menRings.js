...
window.onload = async () => {
  ...
  logEvent(analytics, 'view_menRings_page', {
    page_name: 'Men Rings Collection'
  });

  if (typeof window.fetchMenRingItems !== "function") {
  ...
  const fetchItems = async () => {
    ...
    try {
      const jewelryItems = await window.fetchMenRingItems(currentPage, itemsPerPage);
