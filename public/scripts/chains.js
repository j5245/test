...
window.onload = async () => {
  ...
  logEvent(analytics, 'view_chains_page', {
    page_name: 'Chains Collection'
  });

  if (typeof window.fetchChainItems !== "function") {
  ...
  const fetchItems = async () => {
    ...
    try {
      const jewelryItems = await window.fetchChainItems(currentPage, itemsPerPage);
