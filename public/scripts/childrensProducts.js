...
window.onload = async () => {
  ...
  logEvent(analytics, 'view_childrenProducts_page', {
    page_name: 'Childrens Products Collection'
  });

  if (typeof window.fetchChildrenProductItems !== "function") {
  ...
  const fetchItems = async () => {
    ...
    try {
      const jewelryItems = await window.fetchChildrenProductItems(currentPage, itemsPerPage);
