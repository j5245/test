...
window.onload = async () => {
  ...
  logEvent(analytics, 'view_bracelets_page', {
    page_name: 'Bracelets Collection'
  });

  if (typeof window.fetchBraceletItems !== "function") {
  ...
  const fetchItems = async () => {
    ...
    try {
      const jewelryItems = await window.fetchBraceletItems(currentPage, itemsPerPage);
