...
window.onload = async () => {
  ...
  logEvent(analytics, 'view_womenRing_page', {
    page_name: 'Women Rings Collection'
  });

  if (typeof window.fetchWomenRingItems !== "function") {
  ...
  const fetchItems = async () => {
    ...
    try {
      const jewelryItems = await window.fetchWomenRingItems(currentPage, itemsPerPage);
