...
window.onload = async () => {
  ...
  logEvent(analytics, 'view_earrings_page', {
    page_name: 'Earrings Collection'
  });

  if (typeof window.fetchEarringItems !== "function") {
  ...
  const fetchItems = async () => {
    ...
    try {
      const jewelryItems = await window.fetchEarringItems(currentPage, itemsPerPage);
