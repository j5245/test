...
window.onload = async () => {
  ...
  logEvent(analytics, 'view_necklaces_page', {
    page_name: 'Necklaces Collection'
  });

  if (typeof window.fetchNecklaceItems !== "function") {
  ...
  const fetchItems = async () => {
    ...
    try {
      const jewelryItems = await window.fetchNecklaceItems(currentPage, itemsPerPage);
