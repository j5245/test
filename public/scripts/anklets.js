...
window.onload = async () => {
  ...
  logEvent(analytics, 'view_anklets_page', {
    page_name: 'Anklets Collection'
  });

  if (typeof window.fetchAnkletItems !== "function") {
  ...
  const fetchItems = async () => {
    ...
    try {
      const jewelryItems = await window.fetchAnkletItems(currentPage, itemsPerPage);
